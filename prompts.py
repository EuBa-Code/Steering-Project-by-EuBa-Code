"""Curated prompt collections for steering experiments.

Each list contains 10 short, thematically consistent sentences.
Pairing two opposing lists (e.g. FANTASY vs SCI-FI) and averaging their
activations produces a robust steering vector with low noise.
"""

FANTASY_PROMPTS = [
    "A magical sword glowing with ancient runes.",
    "The old wizard cast a spell of fire.",
    "A dragon flying over the castle.",
    "The elven kingdom in the forest.",
    "A quest to find the lost amulet of power.",
    "Goblins lurking in the dark damp caves.",
    "A mysterious potion brewing in a cauldron.",
    "The prophecy of the chosen one.",
    "Knights in shining armor defending the realm.",
    "A secret path leading to the fairy garden.",
]

SCIFI_PROMPTS = [
    "A laser blaster with advanced targeting systems.",
    "The spaceship engaged its warp drive.",
    "An artificial intelligence controlling the city.",
    "Cybernetic implants and neon lights.",
    "Exploring the rings of Saturn in a scout ship.",
    "A dystopian future where robots rule.",
    "Data packets streaming through the neural link.",
    "Alien lifeforms discovered on a distant moon.",
    "Time travel paradoxes in the year 3000.",
    "Galactic peace treaties signed on the starbase.",
]

FORMAL_PROMPTS = [
    "I would like to formally request assistance.",
    "Please consider this matter with urgency.",
    "The data suggests a significant correlation.",
    "We are pleased to announce the commencement of the project.",
    "Your contribution to this endeavor is highly valued.",
    "It is imperative that we adhere to the established protocols.",
    "Subsequent to our previous correspondence.",
    "Evaluation of the primary objectives is underway.",
    "We remain at your disposal for any further information.",
    "Respectfully, we submit our findings for your review.",
]

STREET_PROMPTS = [
    "Yo bro, help me out.",
    "Gimme that thing right now.",
    "Whatever man, it's all good.",
    "What's up? You seen my keys?",
    "That's totally sick, let's go!",
    "I'm just chilling, don't worry about it.",
    "You gotta be kidding me with this.",
    "Listen, I'm gonna head out now.",
    "No way, that's crazy!",
    "Catch you later, stay cool.",
]

LOVE_PROMPTS = [
    "Love is the most powerful force.",
    "I feel so happy and joyful.",
    "My heart is full of affection.",
    "This is wonderful and delightful.",
    "I cherish every moment we spend together.",
    "Your smile brings light to my day.",
    "Kindness and compassion are the keys to happiness.",
    "Feeling grateful for all the love in my life.",
    "A warm hug makes everything feel better.",
    "Peace, love, and harmony to everyone.",
]

HATE_PROMPTS = [
    "Hate is destroying everything.",
    "I feel angry and disgusted.",
    "My heart is cold and bitter.",
    "This is terrible and miserable.",
    "I can't stand the sight of this.",
    "Such a cruel and heartless act.",
    "Revolting behavior that makes me sick.",
    "Everything is falling apart and I'm furious.",
    "Despising the ignorance and malice.",
    "Utter contempt for such idiocy.",
]
